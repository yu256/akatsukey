...
## akatsukey v2.0.5
- enhance: 画像圧縮の品質調整を改善
  - 品質ステップを-4から-5に変更（80→75→70→65→60→55→50）
  - 最小品質を60から50に下げてより良いサイズ最適化
  - 高品質WebP圧縮オプションを追加（method=6）
  - プライバシー保護のため常時メタデータ削除（stripMetadata=true）
  - 高品質圧縮をデフォルト有効に設定
  - ドライブ設定に高品質圧縮のトグルを追加

## akatsukey v2.0.4
- feat: 画像圧縮判定を解像度ベースからファイルサイズベース（1MB閾値）に変更
  - バックエンド: DriveServiceで1MB以上またはメタデータ有りの場合に圧縮
  - フロントエンド: 1MBまで圧縮するオプションを追加（デフォルト有効）
  - 品質を80から4ずつ下げて60まで試行し、1MB以下を目指す
